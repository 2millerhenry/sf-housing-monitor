...
if [ -x "$APP_ROOT/tools/doctor.sh" ]; then
  ...
  /usr/bin/osascript -e 'display dialog "SF Housing Monitor is not installed yet. Run Install SF Housing Monitor first." buttons {"OK"} default button "OK" with icon caution'
fi
